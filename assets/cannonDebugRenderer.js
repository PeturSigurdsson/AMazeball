...
export default class CannonDebugRenderer{
  ...
  update = () => {
    ...
    for(
      var i = meshIndex; i < meshes.length; i++
    ){
      var mesh = meshes[i];
      if(mesh){
  ...
  _createMesh = (shape) =>{
    ...
    this.tmpVec0 = this.tmpVec0 ||
      new CANNON.Vec3();
    this.tmpVec1 = this.tmpVec1 ||
      new CANNON.Vec3();
    this.tmpVec2 = this.tmpVec2 ||
      new CANNON.Vec3();
